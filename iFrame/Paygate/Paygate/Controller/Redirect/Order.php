...
class Order extends \Paygate\Paygate\Controller\AbstractPaygate
{
    ...
    public function setPaymentMethod()
    {
        ...
        $quote->setPaymentMethod( 'paygate' );
        $quote->setInventoryProcessed( false );

        // Set Sales Order Payment
        $quote->getPayment()->importData( ['method' => 'paygate'] );
        // Now Save Quote
        $quote->save();
    ...
    public function getStandardCheckoutFormFieldss( $order )
    {
        ...
        if ( $model->getConfigData( 'test_mode' ) != '1' ) {
            $paygateId     = $model->getConfigData( 'paygate_id' );
            $encryptionKey = $model->getConfigData( 'encryption_key' );
        } else {
